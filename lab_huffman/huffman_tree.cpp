...
// QUESTIO: WHY WE DO NOT USE template <typename T> Done template only needed in when I create something
                            ...
                            queue<TreeNode*>& mergeQueue)
{
    TreeNode* smallest = NULL;
     ...
     else {// QUESTIO: why can I call getFrequency function? Done
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned i = 0; i< frequencies.size(); i++) {
      TreeNode* temp1 = new TreeNode(frequencies[i]);
    ...
    //   singleQueue.push(*it);
    // }
    TreeNode* smallNode1 = NULL;
    TreeNode* smallNode2 = NULL;
    // int sum = singleQueue.size() + mergeQueue.size(); //Never change the value
    while ((singleQueue.size() + mergeQueue.size()) != 1) {//QUESTIO: if == 1, how can I set the left and right pointer?
      smallNode1 = removeSmallest(singleQueue, mergeQueue);
      smallNode2 = removeSmallest(singleQueue, mergeQueue);
      // if (smallNode1 != NULL && smallNode2 != NULL) {
        char newChar1 = smallNode1->freq.getCharacter() + smallNode1->freq.getCharacter();
        int newFreq1 =  smallNode1->freq.getFrequency() + smallNode2->freq.getFrequency();// QUESTIO?? SEGFAULT LIne 132&133
        //QUESTION: How to use gdb to print out the values?
        Frequency newFreq = Frequency(newChar1, newFreq1);
        TreeNode* newNode = new TreeNode(newFreq1);
        // QUESTIO: TreeNode* newNode = new TreeNode(newFreq) {
        //     theFreq = newFreq;
        //     left = smallNode1;
        //     right = smallNode2;
        //   }
          newNode->left = smallNode1;
          newNode->right = smallNode2;
          mergeQueue.push(newNode);
          //QUESTIO: where the while loop exit? at the check sentence
      // }
    }
...
{ //QUESTIO: WHAT that function means? enconde the tree (encode was used to encode a string)
    /**
     * @todo Your code here!
     *
     ...
     */
     ...
     if (current->left == NULL && current->right == NULL) {
       ...
       bfile.writeBit(0);
       if(current->left != NULL) //QUESTIO: do i need to consider the null? LAB
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        ...
        if(bfile.getNextBit()) {
          char newChar = bfile.getNextByte();
          Frequency newF = Frequency(newChar, 0); //QUESTION: frequency = 0?
          TreeNode* newTN = new TreeNode(newF);
          return newTN;
        }
        else {
          Frequency newF = Frequency(0);
          TreeNode* newTN2 = new TreeNode(newF);
          newTN2->left = readTree(bfile);
          newTN2->right = readTree(bfile);
          return newTN2;
        }
      }
